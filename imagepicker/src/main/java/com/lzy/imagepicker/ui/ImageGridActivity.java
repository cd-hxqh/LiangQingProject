...
public class ImageGridActivity extends ImageBaseActivity implements ImageDataSource.OnImagesLoadedListener, ImageGridAdapter.OnImageItemClickListener, ImagePicker.OnImageSelectedListener, View.OnClickListener {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.i(TAG,"requestCode="+requestCode+",resultCode="+",data="+data);
        if (data != null) {
            ...
            if (resultCode == RESULT_OK && requestCode == ImagePicker.REQUEST_CODE_TAKE) {
                Log.i(TAG,"file="+imagePicker.getTakeImageFile()+",path="+imagePicker.getTakeImageFile().getAbsolutePath());
                //发送广播通知图片增加了
                ...
                Log.i(TAG,"is="+imagePicker.isCrop());
                if (imagePicker.isCrop()) {
                    Intent intent = new Intent(ImageGridActivity.this, ImageCropActivity.class);
                    startActivityForResult(intent, ImagePicker.REQUEST_CODE_CROP);  //单选需要裁剪，进入裁剪界面
                } else {
                    Log.i(TAG,"A="+imagePicker.getSelectedImages().get(0));
                    Intent intent = new Intent();
                    intent.putExtra(ImagePicker.EXTRA_RESULT_ITEMS, imagePicker.getSelectedImages());
                    setResult(ImagePicker.RESULT_CODE_ITEMS, intent);   //单选不需要裁剪，返回数据
                    finish();
